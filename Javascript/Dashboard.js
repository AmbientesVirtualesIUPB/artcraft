document.addEventListener("mousemove", (event) => {
    const threshold = 250; // Distancia umbral para activar el parallax (en píxeles)
    const x = event.pageX;
    const y = event.pageY;

    // Seleccionamos todos los elementos con las clases .parallaxC1 y .parallaxC2
    document.querySelectorAll(".parallaxC1, .parallaxC2").forEach((element) => {
        ...
        if (distance < threshold) {
            ...
            const intensity = 4; // Aumentar la intensidad del movimiento

            // Calculamos el desplazamiento en función de la proximidad y la intensidad
            const moveX = (distX / 30) * scale * intensity;
            const moveY = (distY / 30) * scale * intensity;
